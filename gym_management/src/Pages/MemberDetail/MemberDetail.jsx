import React, { useState , useEffect } from "react";
import ArrowBackIcon from "@mui/icons-material/ArrowBack";
import { useNavigate, useParams } from "react-router-dom";
import Switch from "react-switch";
import axios from 'axios';
import { ToastContainer,toast } from "react-toastify";

function MemberDetail() {
  const navigate = useNavigate();
  const [status, setStatus] = useState("Pending");
  const [renew, setRenew] = useState(false);
  const {id} = useParams();
  const[data,setData] = useState(null);
  const [membership , setMembership] = useState([]);
  const [planMember , setPlanMember] = useState("");

  const handleSwitchButton = async() => {
    let statuss = status === "Active" ? "Pending" : "Active";
    await axios.post(`http://localhost:4000/members/change-status/${id}`,{status :statuss},{withCredentials:true})
    .then((resp)=>{
      console.log(resp)
      toast.success("Status changed")
    })
    .catch(err => {
      console.log(err);
      toast.error("Not able to fetch status")
    })
    setStatus(statuss);
  };

  const fetchData = async() => {
    await axios.get(`http://localhost:4000/members/get-member/${id}`,{withCredentials:true})
    .then((resp) => {
      console.log(resp)
      setData(resp?.data?.member)
      setStatus(resp?.data?.member?.status)
      toast.success("Member data fetched");
    })
    .catch(err => {
      console.log(err);
      toast.error("Failed to fetch Data");
    })
  } 

  const fetchMembership = async() => {
    await axios.get(`http://localhost:4000/plans/get-membership`,{withCredentials:true})
    .then((resp) => {
      console.log(resp)
      setMembership(resp?.data?.membership)
      setPlanMember(resp?.data?.membership[0]._id)
      
    })
    .catch(err => {
      console.log(err);
      toast.error("Failed to fetch Data");
    })
  }
  useEffect(()=>{
    fetchData();
    fetchMembership();
  },[])

  
  const isDateInPast = (inputDate) => {
    const today = new Date();
    const givenDate = new Date(inputDate);

    return givenDate < today;
  }

  const handleOnChangeSelect = (event) => {
    let value = event.target.value;
    setPlanMember(value)
  }

  const handleRenewSaveBtn = async() => {
    await axios.put(`http://localhost:4000/members/update-member-plan/${id}`,{membership : planMember} ,{withCredentials:true})
    .then((resp) => {
      setData(resp.data.member)
      toast.success("Renew Successful")
    })
    .catch(err => {
      console.log(err);
      toast.error("Unable to renew");
    })
  }

  



  return (
    <div className="w-3/4 text-black p-5 ">
      <div
        onClick={() => {
          navigate(-1);
        }}
        className="cursor-pointer border-2 w-fit text-xl font-sans text-white p-2 rounded-xl bg-slate-900"
      >
        <ArrowBackIcon /> Go Back
      </div>

      <div className="mt-10 p-2">
        <div className="w-[100%] h-fit flex">
          <div className="w-1/3 mx-auto">
            <img src={data?.profilePic} alt="" className="w-full mx-auto " />
          </div>
          <div className="w-2/3 mt-5 text-xl p-5">
            <div className="mt-1 mb-2 text-2xl font-semibold">Name: {data?.name}</div>

            <div className="mt-1 mb-2 text-2xl font-semibold">Mobile:{data?.mobileNo}</div>

            <div className="mt-1 mb-2 text-2xl font-semibold">Address:{data?.address}</div>

            <div className="mt-1 mb-2 text-2xl font-semibold">Join Date:{data?.createdAt?.slice(0,10).split('-').reverse().join('-')}</div>

            <div className="mt-1 mb-2 text-2xl font-semibold">
              Next BillDate:{data?.nextBillDate?.slice(0,10).split('-').reverse().join('-')}
            </div>

            <div className="mt-1 mb-2 flex gap-4 text-2xl font-semibold">
              Status :{data?.status}
              <Switch
                onColor="#6366F1"
                checked={status === "Active"}
                onChange={() => {
                  handleSwitchButton();
                }}
              />
            </div>

            {
              isDateInPast(data?.nextBillDate) && 
              <div
              className={`mt-1 rounded-lg p-3 border-2 border-slate-900 text-center w-full md:w-1/2 cursor-pointer
                    hover:text-white hover:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500
                    ${renew && status == "Active" ? "bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white" : "null"}`}
              onClick={() => setRenew((prev) => !prev)}
            >
              Renew
            </div>
            }

            {renew && status === "Active" ? (
              <div className="rounded-lg p-3 mt-5  mb-5 h-fit bg-slate-50 md:w-[100%]">
                <div className="w-full">
                  <div className="my-5">
                    <div>Membership</div>

                    <select 
                    value ={planMember}
                    className="w-full border-2 p-2 rounded-lg"
                    onChange={handleOnChangeSelect}>
                      {
                        membership.map((item,index) => (
                          <option value={item._id}>{item.months} Months Membership</option>
                        )
                        )
                      }
                      
                    </select>

                    <div
                      className={`mt-4 rounded-lg p-3 border-2 border-slate-900 text-center w-1/2 mx-auto cursor-pointer hover:text-white hover:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500
                                `}
                      onClick={() => {handleRenewSaveBtn()}}
                    >
                      Save
                    </div>
                  </div>
                </div>
              </div>
            ) : null}
          </div>
        </div>
      </div>
      <ToastContainer/>
    </div>
  );
}

export default MemberDetail;
